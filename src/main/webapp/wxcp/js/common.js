...
axios.interceptors.response.use(undefined, function (err) {
    ...
    function login(showAlert=true){
        ...
        window.location.href=axios.defaults.baseURL+"wxcp/user/login";
    }
    if (err.response.status === 401 && err.config && !err.config.__isRetryRequest){
        if(!window.isRefreshing){
            if(localStorage.getItem('refreshToken') === "" || localStorage.getItem('refreshToken') ===null){
                // window.open(axios.defaults.baseURL+"wxcp/user/login", "_parent");
                // return;
                login(false);
                return;
            ...
            localStorage.setItem('refreshToken', "");
            /*axios.defaults.headers.common['token'] = token;*/
            return axios.post('wxcp/user/refreshToken').then(function(success){
                console.log(success)
